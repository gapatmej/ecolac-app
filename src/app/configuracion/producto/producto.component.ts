...
export class ProductoComponent implements OnInit {
	...
	public productoDTO = new ProductoDTO();
	public presentacionProductoSeleccionado = new PresentacionProductoDTO();
	public categoriaProductoSeleccionado = new CategoriaProductoDTO();
	public lineaProductoSeleccionado = new LineaProductoDTO();
	...
	public jsonInputProducto = new jsonInput(this.storage.get("token"), ValoresGlobales.R_GES_PRO);
	...
	cargarProductos(){
		this.listaProductos = new Array<ProductoDTO>();
	...
	editar(producto: ProductoDTO){
		...
		this.jsonInputProducto.headerInput.transaccion = ValoresGlobales.S_CONSULTAR_PRODUCTO_001;

		this.jsonInputProducto.bodyInput.data = {ProductoDTO: producto};
		this._productoService.consultarTodos( this.jsonInputProducto )
		.subscribe(response =>{

			this.jsonOutput = response;
			this.productoDTO = this.jsonOutput.bodyOutput.data.ProductoDTO;
			this.editado = true;
			this.presentacionProductoSeleccionado = this.productoDTO.presentacionProducto.PresentacionProductoDTO;
			this.categoriaProductoSeleccionado = this.productoDTO.categoriaProducto.CategoriaProductoDTO;
			this.lineaProductoSeleccionado = this.productoDTO.lineaProducto.LineaProductoDTO;
			this.cargarPresentacionesProducto();
			this.cargarCategoriasProducto();
			this.cargarLineasProducto();
	...
	eliminar(producto: ProductoDTO){
		...
		this.productoDTO.activo = false;
		this.guardar();
	}

	capturarPresentacionProducto(presentacionProducto: PresentacionProductoDTO){
		this.presentacionProductoSeleccionado = presentacionProducto;
		this.productoDTO.presentacionProducto = {PresentacionProductoDTO:this.presentacionProductoSeleccionado}
	}

	capturarCategoriaProducto(categoriaProducto: CategoriaProductoDTO){
		this.categoriaProductoSeleccionado = categoriaProducto;
		this.productoDTO.categoriaProducto = {CategoriaProductoDTO:this.categoriaProductoSeleccionado}
	}

	capturarLineaProducto(lineaProducto: LineaProductoDTO){
		this.lineaProductoSeleccionado = lineaProducto;
		this.productoDTO.lineaProducto = {LineaProductoDTO:this.lineaProductoSeleccionado}
	}
	...
	readThis(inputValue: any): void {
		...
		myReader.onloadend = (e) => {
			this.productoDTO.foto = myReader.result.split(",")[1];
		}
		myReader.readAsDataURL(file);
	}

	sumarCantidad(){
		this.productoDTO.stock = parseInt(this.productoDTO.stock) + 1;
	}

	restarCantidad(){
		if(this.productoDTO.stock == 0)
			return;
	...
	exportar(){
		UtilitariosComponent.exportar('Productos.xlsx', this.listaProductos);
	}
